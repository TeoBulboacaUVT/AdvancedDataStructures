...
struct RBTree {
	...
	/**
	 * predecessor(w) returns the predecessor of node w in the
	 * RB-tree to which node w belongs
	 */
	...
	/*
	RBDeleteFixup(x) restores the properties of the RB-tree after deleting a node and replacing it with node x. 
	 * It is called by del(RBNode* z) after deleting z and replacing it with x.
	 *
	*/
	...
	/*
	* RBInsertFixup(z) restores the properties of the RB-tree after inserting node z. 
	 * It is called by RBInsert(z) after inserting z and coloring it red.
	*
	*/
	...
	// auxiliary method - black-height of the node x
	int bh(RBNode* x) {
		if (isNil(x)) return 0;
		int leftBh = bh(x->left);
		int rightBh = bh(x->right);
		if (leftBh != rightBh) {
			cerr << "Error: black-height property violated at node " << x->toString() << endl;
			exit(1);
		}
		return leftBh + ((x->col == RBNode::BLACK) ? 1 : 0);
	}
...
int maxBlackKey(RBNode* x, int maxKey) {
    if (isNil(x)) return maxKey;
    
    if (x->col == RBNode::BLACK && x->key > maxKey) {
        maxKey = x->key;
    }
    
    maxKey = maxBlackKey(x->left, maxKey);
    maxKey = maxBlackKey(x->right, maxKey);
    
    return maxKey;
}

int maxBlackKeyCall() {
    return maxBlackKey(root, -1000);
}
...
int maxRedKey(RBNode* x, int maxKey) {
    if (isNil(x)) return maxKey;
    
    if (x->col == RBNode::RED && x->key > maxKey) {
        maxKey = x->key;
    }
    
    maxKey = maxRedKey(x->left, maxKey);
    maxKey = maxRedKey(x->right, maxKey);
    
    return maxKey;
}

int maxRedKeyCall() {
    return maxRedKey(root, -1000);
}
